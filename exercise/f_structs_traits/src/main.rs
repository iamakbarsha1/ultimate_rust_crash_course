...
impl Bite for Carrot {
    fn bite(self: &mut Self) {
        ...
        self.percent_left *= 0.5;
    }
}

// Generic function for anything that implements Bite
fn bunny_nibbles<T: Bite + std::fmt::Debug>(item: &mut T) {
    item.bite();
    item.bite();
    // item.bite();
    println!("Bunny nibbles for awhile: {:?}", item);
}
